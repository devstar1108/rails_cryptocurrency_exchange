class ApplicationController < ActionController::Base
  ...
  before_action :set_timezone, :set_gon, :set_markets, :touch_accounts, :get_coin_costs, :set_language
  ...
  def get_coin_costs
    ...
    last_retrived = Time.now.to_datetime.ago(300)
    @btc_price = CoinPrice.where("currency=? and created_at>?", "USD", last_retrived.to_s).order('created_at DESC').first

    if @btc_price.nil?
      @coin_costs = JSON.parse(open("https://api.bitcoinaverage.com/ticker/global/all").read, symbolize_names: true)

      @btc_price = CoinPrice.new
      @btc_price.currency = "USD"
      @btc_price.ask = @coin_costs[:USD][:ask]
      @btc_price.bid = @coin_costs[:USD][:bid]
      @btc_price.avg_24h = @coin_costs[:USD][:"24h_avg"]
      @btc_price.volume_btc = @coin_costs[:USD][:volume_btc]
      @btc_price.volume_percent = @coin_costs[:USD][:volume_percent]
      @btc_price.save
    end
  ...
  def phone_verified?
    current_user.sms_two_factor.activated?
  end
  ...
  def set_gon
    ...
    gon.base_currency = Currency.first.code
    gon.fiat_currency = ""
  ...
  def set_language
    if cookies[:crycen_locale] && I18n.available_locales.include?(cookies[:crycen_locale].to_sym)
      l = cookies[:crycen_locale].to_sym
    else
