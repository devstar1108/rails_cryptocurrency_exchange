class AccountVersion < ActiveRecord::Base
  ...
  def self.order_by_in
    ret = "CASE"
    ret << " WHEN balance + locked > 0 THEN balance + locked ELSE 0 END"
  end

  def self.order_by_out
    ret = "CASE"
    ret << " WHEN balance + locked < 0 THEN balance + locked ELSE 0 END"
  end

  scope :order_by_income, -> { order(order_by_in) }
  scope :order_by_expenses, -> { order(order_by_out) }
