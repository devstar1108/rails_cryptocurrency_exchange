class Withdraw < ActiveRecord::Base
  ...
  UNPROCESSED_STATES = [:accepted, :processing, :almost_done]
  ...
  scope :unprocessed_amount, -> (currency) { with_currency(currency).where(aasm_state: UNPROCESSED_STATES).sum(:sum) }
  scope :done_total_amount, -> { where(aasm_state: :done).sum(:amount) }
  scope :done_total_fee, -> { where(aasm_state: :done).sum(:fee) }
  ...
  def recheck_and_withdraw

    return unless almost_done?

    return 'Insufficient Balance. Please try again later!' if CoinRPC[currency].getbalance.to_d < sum

    fee = [fee.to_f || channel.try(:fee) || 0.0005, 0.1].min

    CoinRPC[currency].settxfee fee
    txid = CoinRPC[currency].sendtoaddress fund_uid, amount.to_f

    update_column :txid, txid
    ...
    return 'Recheck and withdraw successfully!'
